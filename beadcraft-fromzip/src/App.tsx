...
export default function App() {
  ...
  const [savedProjects, setSavedProjects] = useState<SavedProject[]>(() => {
    const saved = localStorage.getItem('beadcraft_projects');
    return saved ? JSON.parse(saved) : [];
  });
  const [likedPatternIds, setLikedPatternIds] = useState<string[]>(() => {
    const saved = localStorage.getItem('beadcraft_likes');
    return saved ? JSON.parse(saved) : [];
  });
  const [followedAuthors, setFollowedAuthors] = useState<string[]>(() => {
    const saved = localStorage.getItem('beadcraft_following');
    return saved ? JSON.parse(saved) : [];
  });

  const handleToggleLike = (id: string) => {
    const updated = likedPatternIds.includes(id)
      ? likedPatternIds.filter(pId => pId !== id)
      : [...likedPatternIds, id];
    
    setLikedPatternIds(updated);
    localStorage.setItem('beadcraft_likes', JSON.stringify(updated));
  };

  const handleToggleFollow = (authorName: string) => {
    const updated = followedAuthors.includes(authorName)
      ? followedAuthors.filter(name => name !== authorName)
      : [...followedAuthors, authorName];
    
    setFollowedAuthors(updated);
    localStorage.setItem('beadcraft_following', JSON.stringify(updated));
  };

  const handleSaveProject = (projectData: Omit<SavedProject, 'updatedAt'>) => {
    ...
    localStorage.setItem('beadcraft_projects', JSON.stringify(updated));
  };

  const handleDeleteProjects = (ids: string[]) => {
    const updated = savedProjects.filter(p => !ids.includes(p.id));
    setSavedProjects(updated);
    localStorage.setItem('beadcraft_projects', JSON.stringify(updated));
  };
